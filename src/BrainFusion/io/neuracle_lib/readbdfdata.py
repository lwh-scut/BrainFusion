...
def readbdfdata(filename, pathname):
    ...
    if 'edf' in filename[0]:  ## DSI
        ...
        data_file = filename[0] if 'data' in filename[0] or 'eeg' in filename[0] or 'emg' in filename[0] else filename[
            1]
        evt_file = filename[0] if data_file == filename[1] else filename[1]
        raw = mne.io.read_raw_bdf(os.path.join(pathname[0], data_file), preload=False)
        ch_names = raw.info['ch_names']
        ...
        ## read events
        try:
            annotationData = mne.io.read_raw_bdf(os.path.join(pathname[0], evt_file))
            ...
            try:
                ...
                desc = np.array([i for i in description])
